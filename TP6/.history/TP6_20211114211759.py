    ...
    for filename in sorted(os.listdir(BNC_folder)):
        ...
        with open(BNC_folder + filename, 'r', encoding="utf8") as f:
            ...
            for s in tqdm(sentences):
                ...
                # print(i)
                if i%1000==0 : pd.DataFrame(all_tuples, columns=['subject_lemma', 'verb_lemma', 'object_lemma']).to_csv(f'all_tuples{model_name[11:]}_2.csv',encoding='utf-8',index=False)
                if i%1000==0 : pd.DataFrame(all_tuples, columns=['subject_lemma', 'verb_lemma', 'object_lemma']).to_csv(f'nb_triplet_{model_name[11:]}_2.csv',encoding='utf-8',index=False)
                
                if i == max_sentences: break  # stop at 50000 phrases 2/2

    pd.DataFrame(all_tuples, columns=['subject_lemma', 'verb_lemma', 'object_lemma']).to_csv(f'all_tuples{model_name[11:]}_2.csv',encoding='utf-8',index=False)
    pd.DataFrame(all_tuples, columns=['subject_lemma', 'verb_lemma', 'object_lemma']).to_csv(f'nb_triplet_{model_name[11:]}_2.csv',encoding='utf-8',index=False)
